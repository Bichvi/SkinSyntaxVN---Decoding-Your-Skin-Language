...
function frontend_root(): string {
    return defined('FRONTEND_ROOT') ? (string)FRONTEND_ROOT : dirname(__DIR__, 2) . '/../frontend';
}

function frontend_view_path(string $relative = ''): string {
    $base = rtrim(frontend_root(), '/\\') . '/views';
    if ($relative === '') {
        return $base;
    }
...
function require_frontend_view(string $relativePath, array $vars = []): void {
    if ($vars !== []) {
        extract($vars, EXTR_SKIP);
    }
    require frontend_view_path($relativePath);
}
