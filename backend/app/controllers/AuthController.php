...
class AuthController {
    ...
    private function render(string $view, array $data = []) {
        require_frontend_view('layouts/header.php', $data);
        require_frontend_view($view . '.php', $data);
        require_frontend_view('layouts/footer.php', $data);
    }

    private function appBaseUrl(): string {
        $configuredAppUrl = defined('APP_URL') ? trim((string)APP_URL) : '';
        if ($configuredAppUrl !== '') {
            return rtrim($configuredAppUrl, '/') . rtrim((string)BASE_URL, '/');
        }
    ...
    private function resolveOAuthRedirectUri(string $provider): string {
        ...
        return $this->buildRouteUrl('auth_social_callback');
    }
    ...
    private function storeSignupOtp(string $email, string $otpCode): void {
    ...
    private function loginCustomerSession(array $user): void {
        $id = $user['id'] ?? $user['ma_nd'] ?? null;
        if ($id === null && isset($user['_id'])) {
            $id = (string) $user['_id'];
        }
    ...
    public function authSocialCallback(): void {
        $provider = strtolower(trim((string)($_GET['provider'] ?? $_SESSION['oauth_provider'] ?? '')));
    ...
    public function guiOtpDangKy(): void {
        ...
        $demoMode = MailService::isDemoMode();
        $emailSent = false;

        if (!$demoMode) {
            $emailSent = $this->sendSignupOtpEmail($email, $otpCode);
            if (!$emailSent) {
                $this->jsonResponse([
                    'ok' => false,
                    'message' => 'Không gửi được email OTP. Cấu hình SMTP trong .env hoặc bật MAIL_DEMO_MODE=1 để demo.',
                ], 500);
            }
        }

        $this->storeSignupOtp($email, $otpCode);

        $response = [
            'ok' => true,
            'message' => $demoMode
                ? 'Chế độ demo: dùng mã OTP hiển thị bên dưới (10 phút).'
                : 'OTP đã được gửi tới email của bạn. Mã có hiệu lực trong 10 phút.',
        ];
