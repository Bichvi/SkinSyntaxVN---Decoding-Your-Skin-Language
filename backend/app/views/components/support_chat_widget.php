...
if (is_logged_in() && current_role() === 'khach_hang') {
  ...
  // Get PDO connection - ensure it exists
  global $pdo;
  if (empty($pdo)) {
  ...
  if ($supportChatCustomerId > 0) {
    ...
    $supportChatUnreadCount = $supportChatModel->getUnreadStaffRepliesCount($supportChatCustomerId);
...
<?php if ($supportChatEnabled): ?>
  <div class="support-chat-widget" data-support-chat-widget>
    <button class="support-chat-widget__trigger" type="button" data-support-chat-toggle aria-expanded="false" aria-controls="supportChatPanel" title="Mở chat hỗ trợ">
      ...
      <?php if ($supportChatUnreadCount > 0): ?>
        <span class="support-chat-widget__badge"><?php echo $supportChatUnreadCount; ?></span>
      <?php endif; ?>
    </button>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      var openWidget = function () {
        ...
        window.requestAnimationFrame(function () {
          ...
          emitLayoutState();
          // Mark chat as read
          fetch('<?= BASE_URL ?>/index.php?r=mark_chat_read', {
            method: 'POST',
            headers: {
            ...
            if (response.ok) {
              ...
              var badge = widget.querySelector('.support-chat-widget__badge');
              if (badge) {
                badge.remove();
              }
            }
