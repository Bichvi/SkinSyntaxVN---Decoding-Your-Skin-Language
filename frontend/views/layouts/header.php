...
<?php if (!is_logged_in()): ?>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      var registerOtpStorageKey = 'skinsyntaxRegisterOtp';
      ...
      var persistRegisterOtp = function (emailValue, otpCode) {
        if (typeof sessionStorage === 'undefined' || !emailValue || !otpCode) {
          return;
        }
        sessionStorage.setItem(registerOtpStorageKey, JSON.stringify({
      ...
      var restoreRegisterOtpHint = function () {
        if (!registerOtpHint || !registerEmailInput || typeof sessionStorage === 'undefined') {
          return;
        }

        var raw = sessionStorage.getItem(registerOtpStorageKey);
        if (!raw) {
          return;
        }
        ...
        try {
          ...
          var savedEmail = String(saved.email || '').trim().toLowerCase();
          var savedOtp = String(saved.otp || '').trim();
          var currentEmail = String(registerEmailInput.value || '').trim().toLowerCase();
          if (savedEmail === '' || savedOtp === '' || savedEmail !== currentEmail) {
            return;
          }
          registerOtpHint.textContent = 'Mã OTP demo: ' + savedOtp + ' — nhập mã này để tiếp tục đăng ký.';
          if (registerOtpInput && registerOtpInput.value.trim() === '') {
            registerOtpInput.value = savedOtp;
          }
        } catch (error) {
        }
      };
        ...
        registerOtpButton.addEventListener('click', function () {
          ...
          window.fetch(signupOtpUrl, {
            ...
            .then(function (result) {
              ...
              if (payload.ok) {
                if (payload.demo_otp) {
                  persistRegisterOtp(emailValue, payload.demo_otp);
                  registerOtpHint.textContent = 'Mã OTP demo: ' + payload.demo_otp + ' — nhập mã này để tiếp tục đăng ký.';
                  if (registerOtpInput) {
                    registerOtpInput.value = String(payload.demo_otp);
                  }
                } else {
      ...
      if (registerEmailInput) {
        registerEmailInput.addEventListener('input', restoreRegisterOtpHint);
        registerEmailInput.addEventListener('change', restoreRegisterOtpHint);
      }
